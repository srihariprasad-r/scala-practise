...
class Student(
    ...
    val lastName: String, 
    private var _quizzes: List[Int] = Nil,  //when a constructor is mutable, make it private so external updates do not happen 
  ...
  def quizzess = _quizzes      // adding type declaration is only for recursive calls
